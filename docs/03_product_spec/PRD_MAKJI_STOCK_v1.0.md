...
> **주의 — Pain은 "우리 서비스의 문제"가 아니라 "MAKJI STOCK이 없던 기존 시장·경쟁사·고객·환경이 이미 겪고 있던 문제"다.** CardFit의 P1~P4가 뱅크샐러드 등 기존 서비스·시장·규제 데이터의 한계였던 것과 동일하게, 아래 4건은 전부 MAKJI STOCK 출시 이전부터 존재하는 시장 상태다. STOCK 자체의 설계에서 새로 발견된 실행 리스크(전환 마찰·사행성 오인 등)는 여기 섞지 않고 2장 페르소나 Pain(C2·C3·N1 등)과 7장 리스크로 분리해서 다룬다.

| # | Pain(기존 시장·경쟁사·고객·환경의 문제) | 실패 KPI(수치화) | 측정 경로 | 연결된 Needs |
| :---: | --- | --- | --- | --- |
| P1 | 기존 프리미엄 베이커리·이커머스는 정가+시즌 프로모션이 전부라, 소비자가 다음 날 다시 확인할 새로운 정보가 없고 재방문 유인이 없다 | 벤치마크 경쟁사 6곳(직접 경쟁자 널담 + 맥락 경쟁자 런던베이글뮤지엄·노티드·성심당·SPC·CJ더마켓) 중 가격 변동·시세를 콘텐츠로 쓰는 곳 **0/6 = 0%**(시장분석08 1절) — 막지와 채널·타깃이 가장 가까운 널담조차 무기는 영양성분·유통 확장이지 가격 경험이 아니다 | 시장분석08 1절 경쟁사 가치 선언 브리핑, RFP 발주 배경(`RFP_ANALYSIS.md` 1장 "정적 이커머스의 한계") | N1 재방문을 부르는 콘텐츠형 가격 경험 |
| P2 | 프리미엄 베이커리 브랜드는 화제성에 의존해 급성장했다가 빠르게 꺾이는 패턴을 반복한다 — 지속 가능한 재방문 장치를 만든 브랜드가 아직 없다 | 런던베이글뮤지엄 최근 매출 둔화, 노티드 매장 급확장 후 고객 이탈(정성, 시장분석01 5절) | 시장분석01 업계 실측 기사(경쟁사 재무·매장 전략 보도) | N2 화제성에 의존하지 않는 재방문 구조 |
| P3 | 헬시플레저 수요는 커지는데 기존 베이커리 채널은 온라인 접근성이 낮고 소비 경험에 재미 요소가 없어, 소비가 프로틴바·그래놀라 같은 대체재로 이탈하고 있다 | 20대 **45.5%**·30대 **51.1%**가 다이어트·간식 대용으로 단백질 제품을 선택(시장분석01 4절) | 시장분석01 4절 대체재 위협 근거(오픈서베이 등 소비 조사 인용) | N3 온라인에서 재미있게 소비할 수 있는 헬시플레저 경험 |
| P4 | 원부자재·환율 변동에 따른 원가 압박이 업계 전반의 마진을 갉아먹고 있다 — 개별 브랜드만의 문제가 아니라 산업 전체가 겪는 기존 문제다 | 2026년 삼립 등 식품업계가 원부자재 부담으로 빵값 평균 **9% 인상**(시장분석01 1절) | 시장분석01 원부자재 가격 인상 뉴스 인용 | N4 원가 변동에도 흔들리지 않는 자동 마진 방어 체계 |

**Needs (실패 KPI 대비 목표 임계치)**

| Needs | 목표 임계치 | 현재 상태(기준선) |
| --- | --- | --- |
| N1 재방문을 부르는 콘텐츠형 가격 경험 | 오전장·오후장 하루 2회 갱신으로 재방문 시 항상 새 정보 노출률 **100%** | 0/6(직접 경쟁자 널담 포함, 기존 경쟁사 중 가격을 콘텐츠로 쓰는 곳 없음) |
| N2 화제성에 의존하지 않는 재방문 구조 | 재방문율 저하 곡선이 업계 화제성 소진 곡선보다 완만 | 미측정 — 출시 전, 업계는 이미 급락 사례(런던베이글·노티드) 보유 |
| N3 온라인에서 재미있게 소비할 수 있는 헬시플레저 경험 | 매일 안정형/공격형 2択 등 선택형 경험 제공 | 기존 채널(오프라인 프리미엄 베이커리, 일반 이커머스)엔 이런 선택 경험 없음 |
| N4 자동 마진 방어 체계 | 총혜택 상한(38% 또는 재정의 값) **위반 0건** | 0건(舊 모델 91일 백테스트 기준, 3구간 모델 재검증 필요 — PRD 10장) |
...
| 목표 | 수치화된 정의 |
| --- | --- |
| G1 (N1 대응) 가격을 매일 바뀌는 콘텐츠로 만든다 | 오전장·오후장 하루 2회 갱신, 전일 대비 변화 정보 노출률 100% |
| G2 (N2 대응) 화제성에 기대지 않는 재방문 구조를 만든다 | 연속 방문·연속 잠금 보상(F-09)으로 재방문율 저하 곡선을 완만화 |
| G3 (N3 대응) 헬시플레저 소비를 재미있는 선택 경험으로 바꾼다 | 매일 안정형·공격형 2択 제공, 선택 없이 결과가 강제되는 케이스 **0건** |
| G4 (N4 대응) 원가 변동에도 마진을 자동으로 지킨다 | 총혜택 상한 위반 **0건**, 위반 시 신규 가격 공개·구매 연결 즉시 중단(GR1, PRD 11장) |

**이 목표가 실제로 성립하려면 먼저 해소해야 하는 실행 조건(STOCK 자체 설계에서 발견된 리스크 — 페르소나 Pain C2·C3·N1, 시장분석06)**

| 실행 조건 | 수치화된 정의 |
| --- | --- |
| 결제 전환 마찰 제거(C3) | STOCK→자사몰 전환 시 재입력 스텝 **0개**, 잠금·보상 코드 자동 인식(F-05) |
| 사행성 아님을 먼저 증명(N1 persona) | 온보딩 최초 화면 노출률 **100%**, 사행성 연상 문구 **0건**(GR2) |
| 잠금은 항상 사용자에게 유리하게 정산(FR-LOCK02) | 오후가가 잠금가보다 높으면 차액 쿠폰, 낮으면 오후가 적용 — 손해 케이스 **0건** |

> 위 목표(G1~G4)는 "왜 이 시장에 이런 서비스가 필요한가"에 대한 답이고, 실행 조건은 "그 답을 우리가 실제로 구현할 때 반드시 넘어야 하는 우리 자신의 장애물"이다 — 둘을 같은 표에 섞지 않는다.
...
기존 대안(경쟁사 6곳 — 직접 경쟁자 널담 + 맥락 경쟁자 5곳, 시장분석08) 대비 MAKJI STOCK의 차별화 지점을 정리한다. CardFit처럼 "판단 소요시간 48배 단축" 같은 하드 넘버 비교는 이 도메인 특성상(체험형 D2C, 초기 단계) 성립하지 않으므로, 시장분석08이 확인한 **"경쟁사 6곳 중 누구도 하지 않는 것"** 기준으로 비교한다.

| 축 | 기존 대안(경쟁사 6곳) | MAKJI STOCK | 검증 근거 |
| --- | :---: | :---: | --- |
| 가격을 콘텐츠로 쓰는가 | **0/6** — 널담은 영양성분·유통 확장, 나머지는 공간·캐릭터·나눔·성분·편의로 어필 | 가격 변동 자체가 매일의 재방문 트리거(F-01) | 시장분석08 1절 경쟁사 가치 선언 브리핑 |
...
> **전제조건(Hygiene Factor)과 차별적 가치는 다르다**: "STOCK→자사몰 매끄러운 전환"(페르소나 Pain C3, 시장분석06)은 네 번째 차별점이 아니라, CJ더마켓이 이미 업계 표준으로 만들어 놓은 **전제조건**이다(시장분석08 2절 각주). 위 표의 세 가치는 이 전제조건이 해결된 뒤에야 의미가 생긴다 — 3장 US-E가 최우선순위인 이유다.
...
### 2.1 "주식시장처럼 설계된 빵마켓" — 유입(마케팅)과 실사용(제품 UX)은 다른 층위다

이 서비스의 정체성은 "빵집에 은유를 얹은 것"이 아니라 **애초에 빵마켓을 주식시장 구조로 설계한 것**이다. 이때 핵심은 페르소나마다 이 컨셉에 "얼마나 익숙한가"가 아니라, **유입 단계와 실사용 단계에서 이 컨셉이 서로 다르게 작동해야 한다**는 것이다.

- **유입 층위(광고·SNS 바이럴)**: "빵값이 주식시장처럼 움직인다"는 컨셉 자체가 금융지식·재테크에 관심 있는 사람들의 호기심·흥미를 후킹하는 소재다. 이 단계의 카피는 낯설고 신기해야 클릭을 유도한다 — 장이 열린다·시세·지수 같은 표현을 오히려 적극적으로 내세운다.
- **실사용 층위(Bread Market·가격 잠금·보상 고르기 화면 자체)**: 일단 들어온 사람의 실제 목적 대부분은 투자 행위가 아니라 "좋은 빵을 사는 것"이다. 여기서는 금융지식 여부와 무관하게 누구에게나 쉽고 직관적으로 이해되는 화면이어야 한다 — 산식·전문용어를 걷어내는 [[feedback_user_facing_copy]] 원칙이 바로 이 층위를 위한 것이다.

이 두 층위를 섞으면 안 된다. 유입 카피의 낯섦(주식 문법)을 화면 안까지 그대로 끌고 오면 순수하게 빵을 사려던 사람(박현주 유형)이 이해 부담으로 이탈하고, 반대로 화면을 지나치게 단순화해 유입 카피가 약속한 신기함을 화면에서 전혀 못 느끼게 하면 금융지식 세그먼트(이도현 유형)를 끌어들인 후킹 자체의 약속이 깨져 재방문 동기가 사라진다.

| 인물 | 유입 단계에서의 위치 | 실사용 화면에서 필요한 것 |
| --- | --- | --- |
| 김소민(Core) | "빵값이 주식처럼 움직인다"는 바이럴 카피에 호기심으로 반응해 클릭(②탐색·유입) | 일단 들어오면 목적은 "쉽게 좋은 빵 고르기" — 화면 자체는 단순해야 함 |
| 박현주(Adjacent) | 이 후킹 카피의 주 타겟이 아님 — 봐도 큰 반응이 없거나 애초에 도달하지 않음 | 화면 난이도가 진입장벽이 되면 안 됨 — 은유를 몰라도 쓸 수 있는 저관여 경로(F-07 알림) 필수 |
| 이도현(Extreme) | 이 후킹이 가장 강하게 작동하는 이상적 타겟 — 광고·바이럴 채널의 핵심 유입원 | 화면 안에서도 "장이 열리는" 감각이 유지돼야 재방문하지만, 그 몰입이 구매로 이어지도록 별도 설계 필요(시장분석06 E2, DOS 음수) |
| 정은숙(Non-user) | 후킹 카피 자체가 역효과 — "이거 투자·도박 광고 아니야?"라는 거부감을 유입 단계에서부터 유발 | 화면(특히 온보딩)에서 "투자가 아니다"를 카피의 신기함보다 먼저 증명해야 함(F-06) |

**F-06(신뢰 고지)의 위치 재정의**: 신뢰 고지는 특정 페르소나의 개인 성향에 대응하는 장치가 아니라, **유입 카피(주식 후킹)에서 실사용 화면(빵 구매)으로 넘어가는 톤 전환점**에 놓여야 하는 장치다 — 광고에서 얻은 흥미를 화면 진입 직후 "이건 투자가 아니라 할인 프로모션"이라는 분명한 전환으로 받아줘야, 금융지식 세그먼트의 흥미와 일반 소비자의 신뢰를 동시에 지킬 수 있다.

**화면 설계 원칙(한 줄 요약)**: 주식 용어는 써도 되지만, 그 용어를 몰라도 바로 다음 문장만 읽으면 뭘 해야 할지 알 수 있어야 한다 — 즉 실사용 화면은 **비금융권 소비자 기준으로 이해되는 것**을 최소 기준선으로 삼는다(F-01·F-06, [[feedback_user_facing_copy]]).
...
- **AC4**: Given 이 전환 경로 개선은 이 PRD의 최우선 과제(페르소나 Pain C3, KSF#1)이므로, When 향후 로드맵을 수립하면, Then 소셜 로그인·쿠폰 자동 인식 등 "재입력 스텝 0개"를 향한 개선안을 Should 등급 이상으로 최우선 배치한다(4장 F-05).
...
| :---: | --- | --- | :---: | --- | --- |
| **F-01** | 오전장·오후장 가격 확인(세션 상태 표시 포함) | FR-H01~03, FR-BM01~07 | **Must** | 경쟁사 0/6(직접 경쟁자 널담 포함)이 가격 변동을 콘텐츠로 쓰지 않음(P1) — 유일한 차별화 축의 기반 | 환율·검색량 데이터 API |
...
| **F-04** | 오늘의 보상 고르기(안정형/공격형) | FR-RW01~05 | **Must** | 위험 성향별 선택형 리워드 — 경쟁사 6곳(널담 포함) 모두 고정형(시장분석08 2절③) | F-01 |
| **F-05** | STOCK→자사몰 전환(원클릭 지향) | FR-PD02, FR-LOCK03 | **Must** | 페르소나 Pain C3(전체 1위, AOS 4.0/DOS 3.6) 직접 대응 — 유일한 매출 실현 관문 | F-02, F-03/F-04 |
| **F-06** | "투자 아님" 신뢰 고지 + 가격 근거 배지 | (신규, US-D) | **Must** | 페르소나 Pain N1·정은숙(공동 1위, AOS 4.0/DOS 3.4) 직접 대응 — 시장 진입 자체의 전제조건 | 없음(문구·UX 자산) |
...
> 핵심 기능은 F-05(STOCK→자사몰 전환)다. F-01~F-04는 이 전환이 성립하기 위한 선행 단계이고, F-06은 진입 자체를 막는 페르소나 Pain N1(정은숙)을 해소하는 전제조건이다. F-05가 "중요도 최상위이면서 동시에 구현 난이도도 최상위"라는 점(시장분석08 VPS Job-Feature Map)이 이 로드맵의 핵심 리스크다.
...
| 축 | 리스크 | 대응 | 상태 |
| --- | --- | --- | :---: |
| 전환 | STOCK→자사몰 수동 연동이 유일한 매출 관문에서 발생(페르소나 Pain C3, KSF#1) | F-05 최우선 배치, 소셜로그인·쿠폰자동인식 로드맵화 | 관리 중 — 전환율 미측정 |
| 규제 | "가격이 오르내인다"는 표현이 사행성으로 오인될 수 있음(페르소나 Pain N1·정은숙, KSF#4) | F-06 신뢰 고지 선(先)배치, F-12 금지어 스캐너, RFP 법무 검토(PRD 12장) | 통제 적용 |
| 데이터 | 환율·검색지수 공식 통합 API 부재 위험 — 수집 실패 시 계산 신뢰도 붕괴 | GR5 최신성 경고·계산 제외 임계치 | 관리 중 |
| 재무 | 환율 상승이 원가와 할인폭을 동시에 악화(P4의 산업 배경이 STOCK 가격 엔진에도 그대로 적용되는 이중 노출) | 38% 하드캡(GR1)으로 마진 최저선 방어 | 통제 적용 |
...
| A2 | "투자가 아니다"라는 고지를 먼저 보여주면 Non-user의 진입 거부가 줄어든다 | 1순위 | F-06의 존재 이유(페르소나 Pain N1·정은숙 해소) | E2 인터뷰(시장분석07 인터뷰 설계 재사용) |
| A3 | STOCK→자사몰 전환 마찰이 실제 이탈로 이어진다 | 1순위 | 페르소나 Pain C3·KSF#1 전체 | E1 베이스라인, E3 전환율 실측 |
...
| 실험 | 검증 질문 | 설계 | 성공 기준 |
| :---: | --- | --- | --- |
| **E1 베이스라인** | STOCK→자사몰 전환 마찰이 실제로 이탈로 이어진다(페르소나 Pain C3·A3) | 베타 출시 직후 전 사용자 대상 퍼널 로그(`lock_completed`/`reward_selected` → 자사몰 쿠폰 사용) | 전환율 분포 확정 — 이후 모든 목표값의 기준선으로 사용 |
...
| --- | --- | --- |
| 기존 경쟁사 누구도 가격 변동을 콘텐츠·신뢰 요소로 쓰지 않는다(시장 Pain P1) | 시장분석01·08 경쟁사 6곳 실측(직접 경쟁자 널담 포함) | 0/6 = 0% |
| 프리미엄 베이커리는 화제성에 의존하다 빠르게 꺾이는 패턴을 반복한다(시장 Pain P2) | 시장분석01 업계 사례(런던베이글뮤지엄·노티드) | 정성 — E5 리텐션 코호트로 정량화 예정 |
| 헬시플레저 수요가 대체재(프로틴바 등)로 이탈하고 있다(시장 Pain P3) | 시장분석01 소비 조사 인용 | 20대 45.5%·30대 51.1% 단백질 제품 선택 |
| 원부자재·환율 원가 압박은 업계 전반의 기존 문제다(시장 Pain P4) | 시장분석01 원부자재 뉴스 + PRD 10장 백테스트(舊 모델) | 삼립 등 9% 가격 인상 사례, 변동성 기여도 환율 97% |
| STOCK→자사몰 전환 마찰이 전체 실행 리스크 중 1위다(페르소나 Pain C3) | 시장분석06 AOS·DOS 채점(정성) | AOS 4.0 / DOS 3.6 |
| 사행성 오인이 진입 거부의 핵심 원인이다(페르소나 Pain N1·정은숙) | 시장분석06 AOS·DOS + 시장분석07 JTBD 가설 | AOS 4.0 / DOS 3.4(공동 1위) |
